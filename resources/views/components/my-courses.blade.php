...
<div class="user-courses-table m-10 p-8 bg-slate-200">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between pb-6">
        <div>
            <span class="text-4xl pb-4 mb-2">My Courses</span>
        </div>
        <div class="flex items-center justify-between ">
            <form class="flex items-center p-2 rounded-md bg-slate-200">

                <input class="bg-gray-50 outline-none ml-1 block " type="text" name="" id="search-course"
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400" id="my-courses-table">
            ...
            <tbody class="text-black">
                @foreach ($user->courses as $course)
                    ...
                    {{ $status == 'confirmed' ? 'bg-green-200' : '' }}">
                        ...
                        <td class="px-6 py-4 text-right text-sm italic">
                            @switch($status)
                                @case('pending')
                                    PENDING
                                @break
                        ...
                        <td class="text-center">
                            @if (!($status == 'confirmed' && $isPastDate($start_date)))
                                <form action="{{ route('unsubscribe', ['user' => $user, 'course' => $course]) }}" method="post">
                                    @csrf
                                    @method('delete')
                                    <button type="submit"
                                        class=" cursor-pointer text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900 mx-auto">
                                        X
                                    </button>
                                </form>
                            @endif
    ...
    @push('scripts')
        @vite(['resources/js/searchCourse.js'])
    @endpush
</div>
